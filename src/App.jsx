...
// NOTE: In your project, restore this import:
// import { checkConnection, createSlot, bookSlot, cancelBooking, completeBooking, getSlot, listSlots, getSlotCount } from "../lib/stellar.js";

/* ─────────────────────────────────────────────
   MOCK STUBS — remove when wiring real stellar.js
   ───────────────────────────────────────────── */
const checkConnection = async () => ({ publicKey: "GAFW2UZ5WCMOOXFQKL6DTP2GKPSJCFJ635L4I4W2FKAEN6OIB75VSCRD" });
const createSlot      = async (p) => ({ ok: true, id: p.id });
const bookSlot        = async (p) => ({ ok: true, id: p.id });
const cancelBooking   = async (p) => ({ ok: true, id: p.id });
const completeBooking = async (p) => ({ ok: true, id: p.id });
const getSlot         = async (id) => ({ id, status: "available", price: "1000", provider: "GAFW...SCRD" });
const listSlots       = async ()   => ["slot1", "slot2"];
const getSlotCount    = async ()   => 2;
...
function LandingPage({ onLaunch }) {
  ...
  const features = [
    ...
    { icon: "🔐", title: "On-Chain Auth", desc: "All bookings are signed via Freighter wallet — no custodial risk, no middlemen." },
    { icon: "✅", title: "State Machine", desc: "Slots flow through available → booked → completed (or cancelled) with full auditability." },
    { icon: "⚡", title: "Soroban Speed", desc: "Built on Stellar's smart contract platform. Sub-second finality, minimal fees." },
    { icon: "🌐", title: "Open Protocol", desc: "Any provider can list slots. Any customer with a Stellar wallet can book." },
    { icon: "📊", title: "Live Queries", desc: "Read slot state instantly with simulated transactions — no wallet needed to browse." },
  ];

  const steps = [
    { n: "01", title: "Connect Wallet", desc: "Link your Freighter wallet with one click to authenticate as a provider or customer." },
    ...
    { n: "04", title: "Complete & Settle", desc: "Mark the service complete. The history is permanently recorded on Stellar." },
  ];
    ...
    <div style={styles.landing}>
      ...
      <section style={styles.hero}>
        ...
        <p style={styles.heroSub}>
          Create service time slots, manage appointments, and settle payments — all on-chain, trustlessly, on the Stellar network.
        </p>
      ...
      <section style={styles.ctaSection}>
        <div style={styles.ctaInner}>
          ...
          <p style={styles.ctaSub}>Connect your Freighter wallet and create your first slot in under a minute.</p>
          <button style={styles.heroPrimary} onClick={onLaunch}>
      ...
      <footer style={styles.footer}>
        ...
        <span style={styles.footerMeta}>Built on Stellar · Soroban · MIT License</span>
      </footer>
    </div>
  );
}
      ...
      <aside style={appStyles.sidebar}>
        <div style={appStyles.sidebarTop}>
          ...
          <div style={appStyles.sidebarBrand}>
            ...
            <div>
              ...
              <div style={appStyles.sidebarNet}>Stellar Testnet</div>
            </div>
          </div>
        </div>
        ...
        <div style={appStyles.walletBox}>
          ...
          <button
            ...
            disabled={isBusy}
          >
            {isBusy_("connect") ? <Spinner /> : connected ? `◉ ${truncateAddr(walletKey)}` : "◎ Connect Freighter"}
          </button>
        </div>
  ...
  return (
    <div>
      ...
      <div style={appStyles.formGrid}>
        <Field label="Provider Address" name="provider" value={form.provider} onChange={setField} placeholder="G…" helper="Your Stellar public key" full />
        <Field label="Service Name" name="serviceName" value={form.serviceName} onChange={setField} />
        <Field label="Price (stroops)" name="price" value={form.price} onChange={setField} type="number" helper="1 XLM = 10,000,000 stroops" />
...
function BookingPanel({ form, setField, onBook, onCancel, onComplete, busyBook, busyCancel, busyComplete, confirmCancel, isBusy }) {
  return (
    <div>
      ...
      <div style={appStyles.formGrid}>
        <Field label="Customer Address" name="customer" value={form.customer} onChange={setField} placeholder="G…" helper="Stellar public key of the customer" full />
        <Field label="Provider Address (for complete)" name="provider" value={form.provider} onChange={setField} placeholder="G…" full />
      </div>
